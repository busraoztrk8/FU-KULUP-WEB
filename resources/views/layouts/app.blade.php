...
<body class="bg-background selection:bg-primary/20 font-body antialiased" data-page="{{ View::getSection('data-page', '') }}">
    ...
    <main class="pt-[80px]">
        {{-- Flash Messages --}}
        <div class="fixed top-[96px] left-1/2 -translate-x-1/2 z-[100] w-full max-w-4xl px-4 space-y-2 pointer-events-none">
            @if(session('warning'))
                <div class="flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 rounded-2xl text-amber-800 text-sm font-medium shadow-2xl pointer-events-auto" x-data="{ show: true }" x-init="setTimeout(() => show = false, 8000)" x-show="show" x-transition>
            ...
            @endif
            @if(session('error'))
                <div class="flex items-start gap-3 p-4 bg-red-50 border border-red-200 rounded-2xl text-red-800 text-sm font-medium shadow-2xl pointer-events-auto" x-data="{ show: true }" x-init="setTimeout(() => show = false, 8000)" x-show="show" x-transition>
            ...
            @endif
            @if(session('success'))
                <div class="flex items-start gap-3 p-4 bg-green-50 border border-green-200 rounded-2xl text-green-800 text-sm font-medium shadow-2xl pointer-events-auto" x-data="{ show: true }" x-init="setTimeout(() => show = false, 8000)" x-show="show" x-transition>
