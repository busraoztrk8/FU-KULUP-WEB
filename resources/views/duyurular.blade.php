...
@section('content')
    ...
    <!-- Hero Section -->
    <section class="relative h-[280px] sm:h-[340px] md:h-[400px] flex items-center overflow-hidden mx-3 sm:mx-4 md:mx-8 mt-4 rounded-2xl md:rounded-3xl shadow-xl bg-gradient-to-br from-amber-600 via-amber-700 to-slate-900">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            ...
            <a href="{{ route('duyuru.detay', $item->slug) }}"
                ...
                <div class="relative h-48 md:h-52 overflow-hidden">
                    ...
                    @endif
                    @if($item->club)
                    <div class="absolute top-3 left-3 bg-amber-600 text-white text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider shadow-lg">
                        {{ $item->club->name }}
                    </div>
                    @endif
                    <div class="absolute top-3 right-3">
                        <div class="w-8 h-8 bg-amber-500 text-white rounded-full flex items-center justify-center shadow-lg">
                ...
                <div class="p-5 md:p-6 card-content">
                    ...
                    <h3 class="font-bold text-slate-800 group-hover:text-amber-600 transition-colors text-lg leading-snug mb-2 line-clamp-2">
                        {{ $item->title }}
                    </h3>
