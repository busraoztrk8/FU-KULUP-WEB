        ...
        <div class="space-y-4 md:space-y-6 flex flex-col items-center md:items-start text-center md:text-left">
            <div>
                ...
                <div class="flex gap-3 md:gap-4">
                    ...
                    @if($instagram)
                        <a class="w-10 h-10 border border-black/5 rounded-full flex items-center justify-center text-primary hover:bg-primary hover:text-white transition-all duration-300 shadow-sm" href="https://instagram.com/{{ $instagram }}" target="_blank" title="Instagram">
                    ...
                    @endif
                    @if($twitter)
                        <a class="w-10 h-10 border border-black/5 rounded-full flex items-center justify-center text-primary hover:bg-primary hover:text-white transition-all duration-300 shadow-sm" href="https://twitter.com/{{ $twitter }}" target="_blank" title="Twitter (X)">
                            <svg class="w-4 h-4 fill-current" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                        </a>
                    @endif
                    @if($facebook)
                        <a class="w-10 h-10 border border-black/5 rounded-full flex items-center justify-center text-primary hover:bg-primary hover:text-white transition-all duration-300 shadow-sm" href="{{ str_contains($facebook, 'http') ? $facebook : 'https://facebook.com/'.$facebook }}" target="_blank" title="Facebook">
            ...
            <div class="space-y-2 mt-2">
                @if($email)
                    <a href="mailto:{{ $email }}" class="flex items-center gap-2 text-xs text-on-surface-variant hover:text-primary transition-colors font-bold">
                        <span class="material-symbols-outlined text-[16px]">mail</span> {{ $email }}
                    </a>
                @endif
                @if($phone)
                    <a href="tel:{{ $phone }}" class="flex items-center gap-2 text-xs text-on-surface-variant hover:text-primary transition-colors font-bold">
            ...
            <div class="pt-4 border-t border-black/5 w-full md:w-auto">
                <p class="font-body text-xs text-on-surface-variant">© {{ date('Y') }} {{ \App\Models\SiteSetting::getVal('site_name', 'Fırat Üniversitesi') }}. Tüm hakları saklıdır.</p>
            </div>
        </div>
    </div>
</footer>
